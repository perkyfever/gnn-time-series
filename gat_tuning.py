...
class GATBlock(nn.Module):
    def __init__(
        ...
        if self.hidden_dim % self.num_heads != 0:
            raise ValueError("Number of heads must divide hidden dimension")
    ...
    def forward(self, graph, features):
        outputs = self.gat(graph, features).reshape(graph.num_nodes(), -1)
...
def run_experiment(config):
    ...
    for epoch in range(config["num_epochs"]):
        ...
        print(f"[train]")
        print(f"loss = {train_output['loss']:.4f}\n")

        print("[valid]")
        print(f"mse = {validation_output['loss']:.4f}")

        print("[test]")
        print(f"mse = {test_output['loss']:.4f}")
