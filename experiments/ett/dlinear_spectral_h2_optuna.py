...
sys.path.append((Path(os.getcwd()).parent.parent / "models").as_posix())
...
from models.gat import GATv2Model
...
def train_step(train_loader, model, optimizer, loss_fn, device):
    ...
    for graph, targets in train_loader:
        ...
        with autocast(device_type="cuda"):
            inputs = graph.ndata["h"].reshape(targets.shape[0], 7, -1)
        ...
        mse_acum += mse.item() / targets[0].numel()
        mae_acum += mae.item() / targets[0].numel()
...
with open("dlinear_study_results_h2.pkl", "wb") as f:
